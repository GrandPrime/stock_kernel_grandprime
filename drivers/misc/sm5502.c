...
static int sm5502_attach_dev(struct sm5502_usbsw *usbsw)
{
	int adc;
	static bool ta_flag;
	int val1, val2, val3, val4, vbus, pump;
	struct sm5502_platform_data *pdata = usbsw->pdata;
	struct i2c_client *client = usbsw->client;
#if defined(CONFIG_VIDEO_MHL_V2)
	/* u8 mhl_ret = 0; */
...
#endif
	pump = i2c_smbus_read_byte_data(client, REG_CHGPUMP_SET);
	if (pump < 0) {
		dev_err(&client->dev, "%s: err %d\n", __func__, pump);
		return pump;
	}
	dev_err(&client->dev,
			"dev1: 0x%x,dev2: 0x%x,dev3: 0x%x,chgpump: 0x%x, Carkit: 0x%x,ADC: 0x%x,Jig: %s\n",
			val1, val2, val3, pump, val4, adc,
