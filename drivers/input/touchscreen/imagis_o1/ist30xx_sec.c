...
get_fw_ver:
	...
	if (ret | (cmd != CMD_ENTER_REG_ACCESS))
		goto get_fw_ver_fail;
...
extern u32 ist30xx_fw_ver;
extern u32 ist30xx_param_ver;
extern u32 ist30xx_sub_ver;
static void fw_update(void *dev_data)
{
	...
	case UMS:
		...
		ist30xx_get_update_info(ts_data, fw, fsize);
		...
		ist30xx_fw_update(data->client, fw, fsize);
...
static void get_threshold(void *dev_data)
{
	...
	threshold = (int)(cfg_buf[0xDC / IST30XX_DATA_LEN] & 0xFFFF);
...
{
	int val = ist30xx_tsp_info.screen.rx;
	char buf[16] = { 0 };
...
{
	int val = ist30xx_tsp_info.screen.tx;
	char buf[16] = { 0 };
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	threshold = (int)((cfg_buf[0xE0 / IST30XX_DATA_LEN] >> 16) & 0xFFFF);
...
int sec_touch_sysfs(struct ist30xx_data *data)
{
	...
	sec_touchscreen = device_create(sec_class, NULL, 0, NULL,
					"sec_touchscreen");
